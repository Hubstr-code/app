 ...
 *
 * Success path:
 *   1. Validate single-use state → recover the Hubstr user who initiated
 *   2. Exchange the code via the platform adapter, normalised into
 *      `NormalizedInstallation`
 *   3. Detect tenant takeover (another Hubstr user already owns this row)
...
export async function messengerOAuthCallback(c: Context): Promise<Response> {
  ...
  if (!config) {
    ...
    return new Response(
      `${definition.name} messenger is not configured on this Hubstr deployment.`,
      { status: 503 },
    );
  }
  ...
  // 3. Detect takeover by another Hubstr user. The tenant install is shared

...
/**
 * Hubstr Skill Store Selectors
 */
export const lobehubSkillStoreSelectors = {
  /**
   * Get all Hubstr Skill server identifiers as a set
   */
  ...
  /**
   * Get all Hubstr Skill servers
   */
  getServers: (s: ToolStoreState): LobehubSkillServer[] => s.lobehubSkillServers || [],

  /**
   * Check if the given identifier is a Hubstr Skill server
   * @param identifier - Provider identifier (e.g., 'linear')
   */
  ...
  /**
   * Get all Hubstr Skill tools as LobeTool format for agent use
   * Converts Hubstr Skill tools into the format expected by ToolNameResolver
   */
    ...
    for (const server of servers) {
      ...
      if (apis.length > 0) {
        tools.push({
          ...
          manifest: {
            api: apis,
            author: 'Hubstr Market',
            homepage: 'https://lobehub.com/market',
            identifier: server.identifier,
            meta: {
              avatar: server.icon || '🔗',
              description: `Hubstr Skill: ${server.name}`,
  ...
  /**
   * Get metadata list for all connected Hubstr Skill servers

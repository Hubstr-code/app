...
export class MessengerSlackBinder implements MessengerPlatformBinder {
  ...
  async handleUnlinkedMessage(ctx: UnlinkedMessageContext): Promise<void> {
    ...
    try {
      ...
      const errorText = 'Hubstr is temporarily unavailable. Please try again in a moment.';
    ...
    if (ctx.channelMentionThreadId) {
      const [, channelId, threadTs] = ctx.channelMentionThreadId.split(':');
      const text =
        "Hi, I'm Hubstr — your AI agent on Slack.\n" +
        `Link your Hubstr account to start chatting: <${verifyUrl}|click here>`;
    ...
    const intro =
      "Hi, I'm Hubstr — your AI agent on Slack.\n" + 'To start, link your Hubstr account.';
  ...
  async notifyLinkSuccess(params: {
    ...
    const headline =
      ':white_check_mark: Linked successfully! Your Hubstr account is now connected.';
   ...
   * `@Hubstr` in a public channel — we want them to get the verify-im URL

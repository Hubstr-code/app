 ...
 *
 * Account model: each `(Hubstr user, platform, tenant_id)` triple has at
 * most one row in `messenger_account_links`, so a single Hubstr user can
...
export class MessengerRouter {
  ...
  private registerHandlers(
    ...
    // listing reviewers also test the `/invite @Hubstr` entry point, so the
  ...
  private buildCommands(): MessengerCommand[] {
    return [
      {
        description: 'Bind your account to Hubstr',
        handler: async (ctx) => {
          ...
          if (ctx.link) {
            await ctx.reply(
              'Your account is already linked to Hubstr. Send /agents to switch the active agent, or /new to start a fresh conversation.',
            );
            return;
          }
          ...
          if (!ctx.isDM && !canEphemeralInChannel) {
            await ctx.reply('Check your DM with Hubstr for the link button.');
          }
        },
        name: 'start',
      },
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            ...
            await ctx.reply('Open your direct message with the Hubstr bot and send `/new` there.');
            return;
          }
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            await ctx.reply(
              'Open your direct message with the Hubstr bot and send `/stop` there.',
            );
            return;
          }
      ...
      {
        description: 'Send feedback directly to the Hubstr team (no AI reply)',
        ...
        handler: async (ctx) => {
          // Feedback is tied to a Hubstr account so the team can follow up;
  ...
  private async runAgentsCommand(ctx: MessengerCommandContext): Promise<void> {
    ...
    if (userAgents.length === 0) {
      await ctx.reply('You have no agents yet. Create one in Hubstr, then come back to /agents.');
      return;
    }
    ...
    if (binder.sendAgentPicker) {
      await binder.sendAgentPicker(chatId, {
        ...
        // channel would broadcast everyone's `Hubstr AI / Claude Code / …`
   ...
   * which Hubstr context a conversation runs in is the active agent's scope;
   ...
   * (inbox/Hubstr AI is pinned first by `fetchUserAgents`) so switching never
  ...
  private scopeSwitchedText(scopeName: string, defaultAgentTitle?: string): string {
    return defaultAgentTitle
      ...
      : `Switched to ${scopeName}. No agents here yet — create one in Hubstr, then /agents.`;
  }
  ...
  private async handleAppHomeOpened(
    ...
    try {
      ...
      const text = activeAgentName
        ? `Welcome to Hubstr! Your active agent is *${activeAgentName}*. Send a message to chat, or use \`/agents\` to switch.`
        : 'Welcome to Hubstr! Send `/agents` to pick an active agent and start chatting.';
   ...
   * itself joins a channel (via `/invite @Hubstr` or being added through the
  ...
  private async handleChannelJoin(
    ...
    const text = [
      ":wave: Hi, I'm *Hubstr* — your AI agent on Slack.",
      '',
      '• Mention me with `@Hubstr <your question>` to chat in this channel.',
      '• First time? Send me a *direct message* to link your Hubstr account.',
   ...
   *  - excludes virtual agents but explicitly keeps the inbox/Hubstr AI agent
   *  - orders by `updatedAt DESC`
   *  - pins inbox/Hubstr AI to the top regardless of updatedAt
   *  - applies the Hubstr AI title fallback (slug='inbox') and a generic
  ...
  private async dispatchToAgent(
    ...
    // Hubstr user; the dispatch only fires for the linked sender. So

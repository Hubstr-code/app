...
export interface AgentSelfIterationCapabilityInput {
  ...
  /**
   * Whether the agent is the product-owned Hubstr AI agent.
   */
  isLobeAiAgent: boolean;
}

/**
 * Checks whether a slug belongs to Hubstr AI.
 *
 * Use when:
 * - Agent Signal must special-case the product-owned default assistant
 * - Code must avoid treating every virtual/builtin agent as Hubstr AI
 *
 * Expects:
 ...
 * - Hubstr AI should be managed by feature flag instead of stored chat config
 *
 * Expects:
 * - `isAgentSelfIterationFeatureEnabled` already includes the user-level feature-flag result
 *
 * Returns:
 * - `true` for Hubstr AI when the feature flag is enabled
 * - `true` for non-Hubstr AI only when `agentSelfIterationEnabled` is true
 */

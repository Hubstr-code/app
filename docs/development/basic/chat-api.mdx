...
description: >-
  Explore the client-server interaction logic of Hubstr Chat API, including
...
Hubstr Chat API in client-server interactions,
...
Hubstr supports three types of tools:
...
abstraction layer in Hubstr for interacting with
...
Agent Runtime (`packages/agent-runtime/`) is Hubstr's

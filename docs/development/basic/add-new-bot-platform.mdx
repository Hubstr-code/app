...
description: >-
  Learn how to add a new bot platform (e.g., Slack, WhatsApp) to Hubstr's
...
This guide walks through the steps to add a new bot platform to Hubstr's channel system. The platform architecture is modular — each platform is a self-contained directory under `src/server/services/bot/platforms/`.
...
export const myPlatform: PlatformDefinition = {
  ...
  documentation: {
    ...
    setupGuideUrl: 'https://hubstr.com/docs/usage/channels/<platform>',
  },
  schema,
...
Follow the structure of existing docs (e.g., `discord.mdx`): Prerequisites → Create App → Configure in Hubstr → Configure Webhooks → Test Connection → Configuration Reference → Troubleshooting.

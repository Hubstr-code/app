---
title: Use Hubstr on Telegram
description: >-
  Link your Hubstr account to the official Hubstr bot on Telegram. Send /start
...
# Use Hubstr on Telegram

Telegram is the simplest Messenger platform: there is one global Hubstr bot, and any Telegram account can DM it. You only need to link your Hubstr account once.
...
In Hubstr, open **Settings → Messenger** and click the **Telegram** card.
...
Either tap **Open in Telegram** in the modal, or scan the QR code with your phone. Telegram opens the official Hubstr bot.
...
Tap the link in the bot's reply. You'll be taken back to Hubstr. Sign in if asked, then choose a **default agent** — every message you DM the bot from now on will route to this agent.

{/* TODO: screenshot — confirm-link page in Hubstr with the agent picker visible */}
...
- **In Hubstr** — open **Settings → Messenger → Telegram** and use the agent picker on your link row.
...
> Disconnecting from Hubstr does not remove the bot from your Telegram chat list — you can manually delete the chat on Telegram if you no longer want to see it.
...
- **"This account is already linked"** — Your Telegram account is bound to a different Hubstr account. Sign in to that Hubstr account to manage the link, or unlink there first.
- **"Another Telegram account is already linked"** — Your Hubstr account already has a Telegram link. Disconnect it in **Settings → Messenger → Telegram** before linking a new Telegram account.

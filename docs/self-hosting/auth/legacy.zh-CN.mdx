...
Hubstr 与 Clerk 做了深度集成，能够为用户提供安全、便捷的登录和注册体验。

在 Hubstr 的环境变量中设置 `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` 和 `CLERK_SECRET_KEY`，即可开启和使用 Clerk。
...
在使用 NextAuth 之前，请先在 Hubstr 的环境变量中设置以下变量：

| 环境变量                             | 类型 | 描述                                                                                                           |
| -------------------------------- | -- | ------------------------------------------------------------------------------------------------------------ |
| `NEXT_PUBLIC_ENABLE_NEXT_AUTH`   | 必选 | 用于启用 NextAuth 服务，设置为 `1` 以启用，更改此项需要重新编译应用。使用 `Hubstr-code/app` 镜像部署的用户已经默认添加了该项配置。                           |
...
允许 Hubstr 在身份提供商中用户信息更新时接收通知。支持的提供商包括 Casdoor 和 Logto。请参考具体提供商文档进行配置。

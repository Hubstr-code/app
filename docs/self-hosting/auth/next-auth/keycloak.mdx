---
title: Configuring Keycloak Authentication Service in Hubstr
description: >-
  Learn how to configure the Keycloak authentication service in Hubstr,
...
# Configuring Keycloak Authentication Service in Hubstr
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with Hubstr via Docker
...
- Your Hubstr database version IP/port is `http://Hubstr_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your Hubstr database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "Hubstr", then click "Create"
  ...
  2. Create a Client
     ...
     - On the "Login settings" page:
       - Valid redirect URIs:
         ...
         - Local network IP deployment: `http://Hubstr_IP:3210/api/auth/callback/keycloak`
         - Public environment: `https://lobe.example.com/api/auth/callback/keycloak`
       - Web origins: Add your Hubstr domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the Hubstr environment variables.

  Configure the Hubstr environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/Hubstr` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/Hubstr` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/Hubstr` for Keycloak deployed in a public environment
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for Hubstr. For Keycloak, fill in `keycloak`.                                                                                                        |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use Hubstr.
</Callout>

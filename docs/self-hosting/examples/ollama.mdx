---
title: Integrating Hubstr with Ollama for Enhanced Language Models
description: >-
  Learn how to configure and deploy Hubstr to leverage Ollama's powerful
  language models locally. Follow the guide to run Ollama and Hubstr on your
  system.
tags:
  - Ollama integration
  - Hubstr configuration
  - Local deployment
  - Language models
  - Ollama usage
---
...
Ollama is a powerful framework for running large language models (LLMs) locally, supporting various language models including Llama 2, Mistral, and more. Now, Hubstr supports integration with Ollama, meaning you can easily use the language models provided by Ollama to enhance your application within Hubstr.

This document will guide you on how to configure and deploy Hubstr to use Ollama:
...
## Running Hubstr Locally

Assuming you have already started the Ollama service locally on port `11434`. Run the following Docker command to start Hubstr locally:

```bash
docker run -d -p 3210:3210 -e OLLAMA_PROXY_URL=http://host.docker.internal:11434 Hubstr-code/app
```

Now, you can use Hubstr to converse with the local LLM.

For more information on using Ollama in Hubstr, please refer to [Ollama Usage](/docs/usage/providers/ollama).

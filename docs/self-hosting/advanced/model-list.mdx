---
title: Customizing Provider Model List in Hubstr for Deployment
description: >-
  Learn how to customize the model list in Hubstr for deployment with the
...
Hubstr supports customizing the model list during deployment. This configuration is done in the environment for each [model provider](/docs/self-hosting/environment-variables/model-provider).
...
docker run -d -p 3210:3210 \
  ...
  --name lobehub-test Hubstr-code/app
```
...
- Confirm the model actually supports the capability in the provider's API (Hubstr cannot enable capabilities the API doesn't provide)
- Verify you are running a recent enough version of Hubstr

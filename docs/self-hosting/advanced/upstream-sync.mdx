---
title: Hubstr Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for Hubstr Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest Hubstr image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running Hubstr Container

  Assuming the Hubstr container is named `lobehub`, use the following commands to stop and remove the currently running Hubstr container:
  ...
  ### Pull the Latest Hubstr Image

  Use the following command to pull the latest Docker image for Hubstr:

  ```fish
  docker pull Hubstr-code/app
  ```

  ### Restart the Docker Container

  Redeploy the Hubstr container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of Hubstr's chat records are stored in your local browser. Therefore, when redeploying Hubstr using Docker, your chat records will not be lost.
</Callout>
...
<Steps>
  ...
  output=$(docker pull Hubstr-code/app:latest 2>&1)
  ...
  echo "$output" | grep -q "Image is up to date for Hubstr-code/app:latest"
  ...
  echo "Started: $(docker run -d --network=host --env-file /path/to/lobe.env --name=lobehub --restart=always Hubstr-code/app)"

  # Print the update time and version
  echo "Update time: $(date)"
  echo "Version: $(docker inspect Hubstr-code/app:latest | grep 'org.opencontainers.image.version' | awk -F'"' '{print $4}')"

  # Clean up unused images
  docker images | grep 'Hubstr-code/app' | grep -v 'latest' | awk '{print $3}' | xargs -r docker rmi > /dev/null 2>&1

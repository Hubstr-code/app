---
title: Deploying Hubstr with Docker Compose
description: >-
  Learn how to deploy the Hubstr service using Docker Compose, including
...
# Deploying Hubstr Server Database Version with Docker Compose
...
<Steps>
  ...
  Visit your Hubstr service at [http://localhost:3210](http://localhost:3210).
</Steps>
...
<Steps>
  ...
  You can access your Hubstr service at `http://your_server_ip:3210`.
</Steps>
...
<Steps>
  ...
  - Domain setup for the Hubstr service: `lobe.example.com`
  ...
  <Callout type="tip">
    **Single Domain Deployment**
    ...
    - Domain setup for the Hubstr service: `lobe.example.com:${PORT1}`
  ...
  You can access your Hubstr service via `https://lobe.example.com`.
</Steps>

## Custom Deployment

This section mainly introduces the configurations that need to be modified to customize the deployment of the Hubstr service in different network environments. Before starting, you can download the [Docker Compose configuration file](https://raw.githubusercontent.com/Hubstr-code/app/HEAD/docker-compose/deploy/docker-compose.yml) and the [environment variable configuration file](https://raw.githubusercontent.com/Hubstr-code/app/HEAD/docker-compose/deploy/.env.example).

```sh
curl -O https://raw.githubusercontent.com/Hubstr-code/app/HEAD/docker-compose/deploy/docker-compose.yml
curl -O https://raw.githubusercontent.com/Hubstr-code/app/HEAD/docker-compose/deploy/.env.example
mv .env.example .env
```

### Prerequisites

Generally, to fully run the Hubstr database version, you will need at least the following three services:

- The Hubstr database version itself
- PostgreSQL database with PGVector plugin
- Object storage service that supports S3 protocol

These services can be combined through self-hosting or online cloud services to meet various deployment needs. In this article, we provide a Docker Compose configuration entirely based on open-source self-hosted services, which can be used directly to start the Hubstr database version or modified to suit your requirements.
...
Hubstr needs to provide a public access URL for object files for the LLM service provider, so you need to configure the S3 Endpoint:
...
For production deployments with a custom domain and HTTPS, configure a reverse proxy in front of Hubstr.
...
[docker-pulls-shield]: https://img.shields.io/docker/pulls/Hubstr-code/app?color=45cc11&labelColor=black&style=flat-square

---
title: Deploying Hubstr Database with Docker
description: >-
  Learn how to deploy the Hubstr server database version using Docker on Linux
  and local machines.
tags:
  - Hubstr
  - Docker
...
Here is the process for deploying the Hubstr server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    components for Hubstr to implement RAG and knowledge base search.
  </Callout>
  ...
  ### Create a file named `hubstr.env` to store environment variables:
  ...
  ```sh
  docker run -it -d -p 3210:3210 --network pg --env-file hubstr.env --name hubstr Hubstr-code/app
  ```
...
The data version of Hubstr also supports direct use on a local Mac/Windows machine.
...
```shell
$ docker run -it -d --name hubstr -p 3210:3210 \

 ...
 * 2. Adding X-Auth-Required header for Hubstr session UNAUTHORIZED errors
 *
 * The X-Auth-Required header allows the desktop app (BackendProxyProtocolManager)
 * to distinguish between real Hubstr session failures (e.g., token expired)
...
export function createResponseMeta({ ctx, errors }: ResponseMetaParams): {
  ...
  // Only set X-Auth-Required for Hubstr session failures, not for Market OAuth failures.

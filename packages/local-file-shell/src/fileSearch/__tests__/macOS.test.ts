...
describe('buildFilenameKeywordExpression', () => {
  ...
  it('splits whitespace-separated keywords into AND-ed substring terms', () => {
    // Critical fix: a free-form keyword string from the LLM (e.g. "Hubstr
    ...
    expect(buildFilenameKeywordExpression('Hubstr Financial Statement')).toBe(
      '(kMDItemFSName == "*Hubstr*"cd && kMDItemFSName == "*Financial*"cd && kMDItemFSName == "*Statement*"cd)',
    );
  });

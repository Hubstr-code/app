...
- User mentions: "SKILL.md", "Hubstr Skills", "skill store", "install skill", "search skill"
- User provides a GitHub link to install a skill (e.g., github.com/xxx/xxx containing SKILL.md)
- User mentions installing from Hubstr marketplace
- User provides Hubstr skill URLs like: \`https://lobehub.com/skills/{identifier}/skill.md\` → extract identifier and use \`importFromMarket\`
...
2. **For Hubstr skill URLs** (e.g., \`https://lobehub.com/skills/{identifier}/skill.md\`):
...
4. If credential doesn't exist:
   - For Hubstr OAuth services (GitHub, Linear, Microsoft, Notion, Twitter) → use \`initiateOAuthConnect\`
...
- **SKILL-FIRST: Any mention of skills, SKILL.md, GitHub skill links, or Hubstr marketplace → activate \`lobe-skill-store\` FIRST, no exceptions.**

...
/**
 * Maps AiHubMix `types` field values to Hubstr AiModelType.
 ...
 * Hubstr's AiModelType and are filtered out before model list processing to
...
/**
 * AiHubMix `types` values that have no corresponding Hubstr AiModelType.
...
/**
 * Map AiHubMix full-catalog API response fields to Hubstr model card fields.
...
const mapAiHubMixModel = (m: any): { [key: string]: any; id: string } => {
  ...
  return {
    ...
    // `structured_outputs` has no corresponding Hubstr model card field and is intentionally omitted.
    ...(featureSet && {

 ...
 * different Hubstr user. Callers (e.g. the messenger router) should surface
...
export class MessengerAccountLinkConflictError extends Error {
  ...
  constructor(existingUserId: string, message?: string) {
    super(message ?? 'IM identity is already linked to another Hubstr user');
...
/**
 * Thrown when the same Hubstr user already has a different IM identity bound
...
export class MessengerAccountLinkModel {
   ...
   * Hubstr user; we throw `MessengerAccountLinkConflictError` instead.
   *
   ...
   * `agentId` — callers pass the scope's default agent (inbox/Hubstr AI) so

...
/**
 * Maps a Hubstr user to a single IM account per platform (e.g. one Telegram
 * account ↔ one Hubstr user). The active agent for that IM session is
...
export const messengerAccountLinks = pgTable(
  ...
  (t) => [
    // One IM account per (platform, tenant) binds to exactly one Hubstr user.
    // The tenant column lets the same Slack user id under two workspaces
    // bind to different Hubstr users — without it, the second workspace
    ...
    // One Hubstr user has at most one IM account per (platform, tenant) —

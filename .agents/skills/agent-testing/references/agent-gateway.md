# Hubstr gateway streaming + tab-switch test harness
...
works for any Hubstr streaming session.
...
- **Never identify tabs by innerText.** The active tab's text embeds a
  ` · <agent name>` suffix, so a search like `'Hubstr Growth'` matches the
  active tab when the active agent happens to be Hubstr Growth — and you
